...
public class HostGetPromotionQueryHandler {
    ...
    public PageResponse<HostPromotionDataResponseDto> handle(HostGetPromotionQuery query) {
        List<KhuyenMai> promotions = promotionRepository.getAll().stream()
                .filter(promotion -> promotion.getNguoiTao().getUserId().equalsIgnoreCase(query.getHostId()))
                ...
                .filter(km -> query.getType() == null || km.getLoaiChietKhau().equalsIgnoreCase(query.getType()))
                .sorted((km1, km2) -> {
        ...
        List<HostPromotionDataResponseDto> promotionDtos = pagedPromotions.stream()
                .map(km -> {
                    ...
                    List<HostHomestayList> homestays = km.getKhuyenMaiPhongs().stream()
                            .map(kmp -> {
                                HostHomestayList homestayDto = new HostHomestayList();
                                Homestay homestay = kmp.getPhong().getHomestay();
                                homestayDto.setId(homestay.getIdHomestay());
                                homestayDto.setHomestayName(homestay.getTenHomestay());
                                return homestayDto;
                            }).distinct()
                            .toList();
                    return PromotionMapper.toHostPromotionDataDto(km, title, homestays);
                })
                .toList();
