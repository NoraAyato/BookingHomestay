...
public interface JpaPhongRepository extends JpaRepository<Phong, String> {
    ...
    @Query("SELECT new com.bookinghomestay.app.application.homestay.dto.RoomDetailResponseDTO(p.maPhong,p.tenPhong,p.soNguoi, lp.tenLoai, p.donGia, cs.nhanPhong, cs.traPhong, cs.huyPhong) "
            +
            "FROM Phong p " +
            "JOIN p.loaiPhong lp " +
            "JOIN p.homestay h " +
