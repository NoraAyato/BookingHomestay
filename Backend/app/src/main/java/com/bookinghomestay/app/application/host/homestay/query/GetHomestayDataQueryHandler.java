    ...
    public PageResponse<HostHomestayDataResponseDto> handle(GetHomestayDataQuery query) {
        ...
        List<HostHomestayDataResponseDto> dtoList = pagedHomestays.stream()
                .map(hs -> {
                    ...
                    int availableRooms = homestayService.countAvailableRooms(hs);
                    List<String> amenities = homestayService.getHomestayAmenities(hs);
                    List<ServiceDataDto> services = hs.getDichVus() != null ? hs.getDichVus().stream()
                            .map(dv -> new ServiceDataDto(dv.getMaDV(), dv.getTenDV(), dv.getDonGia().intValue(),
                                    dv.getMoTa(), dv.getHinhAnh()))
                            .collect(Collectors.toList()) : List.of();
                    return HomestayMapper.toHostHomestayDataResponseDto(hs, averageRating, totalReviews, totalBookings,
                            revenue, availableRooms, amenities, services);
                })
